...
(define salva-nome-tests
  (test-suite
   "teste salva nome"
   (check-equal? (salva-nome "lucas") "lucas")
   (check-equal? (salva-nome "tatsu") "tatsu")
   (check-equal? (salva-nome "123massashi") "123massashi")
   
   )
)

(define simula-cont2 4)
(salva-nome "teste-suite")
(set! simula-cont2 9999999999)
(write (list (string-append (number->string simula-cont2) " " nome )) out)

(salva-nome "teste-suite2")
(set! simula-cont2 999999999)
(write (list (string-append (number->string simula-cont2) " " nome )) out)


(define salva-lista-tests
  (test-suite
   "teste se esta salvando lista"
   (check-equal?  (get-ranking 0) "9999999999 teste-suite")
   (check-equal?  (get-ranking 1) "999999999 teste-suite2")
   )
  )
